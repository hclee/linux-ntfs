		...
		struct inode **ref_vi)
{
	...
	*ref_vi = NULL;
	...
	/*
	 * This is an extent mft record.  Skip writing it from the MFT folio
	 * writeback path entirely.  Dirty extent mft records are written
	 * exclusively by __ntfs_write_inode() via the base inode writeback
	 * path, which calls write_mft_record() for each dirty extent.
	 *
	 * Writing extent records here would require acquiring extent_lock
	 * and/or mrec_lock, which causes an ABBA deadlock with
	 * __ntfs_write_inode() that takes these locks in the opposite order.
	 *
	 * This is safe because:
	 *  - mark_mft_record_dirty(ext_ni) does NOT dirty the MFT folio;
	 *    it sets NInoDirty(ext_ni) and marks the base VFS inode dirty
	 *    via __mark_inode_dirty(I_DIRTY_DATASYNC), triggering the
	 *    __ntfs_write_inode() path.
	 *  - write_mft_record_nolock() copies ni->mrec into the folio and
	 *    submits a bio directly to disk, so any previously written
	 *    extent data in the folio already matches what is on disk.
	 *  - ntfs_evict_big_inode() calls ntfs_commit_inode() before
	 *    freeing extent inodes, ensuring all dirty extents are flushed
	 *    before the base inode leaves the icache.
	 */
