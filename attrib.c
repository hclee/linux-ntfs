		...
		const u8 *val, const u32 val_len, struct ntfs_attr_search_ctx *ctx)
{
	...
	for (;;	a = (struct attr_record *)((u8 *)a + le32_to_cpu(a->length))) {
		...
		/*
		 * Validate attribute's value offset/length so callers can access
		 * the attribute value safely.
		 */
		if (!a->non_resident) {
			u32 min_len;
...
do_next_attr_loop:
		...
		if (!a->non_resident) {
			...
			if (value_length && le32_to_cpu(a->data.resident.value_length) < value_length) {
